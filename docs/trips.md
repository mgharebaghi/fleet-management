...
- `Assigned` requires a persisted Planned execution for every passenger. The
  primary action on the workspace is «شروع سفر».
- `InProgress` is initiated by the «شروع سفر» action (or status update), which
  transitions the TripRequest and all planned executions to `InProgress` atomically.
  `TripExecution.ActualPickupDateTime` remains `null` until entered from the driver
  mission sheet/report (the database has no request-level actual start timestamp column).
- `Completed` requires all child passenger executions to be `Completed`.
- Cancellation is accepted only from `New` or `Assigned` before any execution
  starts. Cancelling the request also cancels remaining Planned children.

Trip execution time semantics:

- `Trip.RequestedPickupDateTime`: Planned/requested pickup time per passenger, set
  during creation, passenger addition, or editing.
- `TripExecution.ActualPickupDateTime`: Actual time passenger was picked up,
  recorded from the returned mission sheet during `InProgress`.
- `TripExecution.ActualDropoffDateTime`: Actual time passenger was dropped off,
  recorded from the returned mission sheet during `InProgress`.
...
- Planned/Cancelled executions cannot carry actual start data.
- InProgress executions allow entering actual pickup time and odometer while
  keeping status in progress, or single-pass completion when both pickup and
  dropoff are entered.
- Completed requires both actual times.
- Reopening the execution modal during InProgress hydrates previously saved
  data and updates the existing row in place (no duplicate executions).
- Assignment is immutable after actual start.
- `trip.Trip.Status` remains nullable and has no invented lifecycle.
...
`/trips/create` is a Presentation-only **progressive 6-step wizard**:

1. **اطلاعات درخواست** — request type, shared locations, schedule, purpose, description.
2. **مسافران** — `TripPassengerSwitcher` tabs with one mounted passenger editor; inactive passengers stay in hidden snapshot fields and remain editable after Back navigation.
3. **راننده و خودرو** — per-passenger eligible assignment selection stored only in wizard state, keyed by the unsaved passenger index.
4. **مسیر** — optional routes and optional ordered points stored only in wizard state. A route may contain zero points; a point requires a Location only after the user explicitly adds it.
5. **برنامه‌ریزی و برگه مأموریت** — previews operational readiness. Voucher links become available from Trip Details only after real request and Trip IDs exist.
6. **مرور و تأیید نهایی** — reviews the full unsaved request; only «ثبت نهایی درخواست» persists and redirects to the Trip Details workspace (`/trips/{id}`).

### Final persistence boundary
- Back, Next, passenger changes, assignment selection, and route editing do not mutate SQL Server.
- Final confirmation sends the complete payload to one Application operation. One Serializable repository transaction generates the server registration time and RequestNo, creates the TripRequest and passenger Trips, maps their generated TripIds to Planned TripExecutions and optional Routes/RoutePoints, then marks the request `Assigned`.
- Any failure rolls back the complete operation; no partial request, Trip, execution, route, or point remains.
- Cancelling, navigating away, closing, or refreshing before final confirmation creates no database record. Unsaved wizard state is intentionally not resumed or stored in browser storage.
- Historical partial requests created by the former flow are not automatically deleted.

## Staff workspace

`/trips/{id}` is a post-creation administrative workspace for tracking progress, managing execution, handling exceptions, and viewing logs. It does not duplicate creation wizard forms. Initial planning and execution preparation are part of the progressive Create Trip wizard, so separate Planning and Execution tabs are removed from the workspace in favor of **five URL-driven tabs** with clean responsive 2-column wrapping on mobile screens and zero horizontal scroll. Only the active tab body is rendered. Tab navigation uses the shared site-wide `LoadingIndicator` during transitions. All tabs are strictly read-only for terminal requests (`Completed`, `Cancelled`).
