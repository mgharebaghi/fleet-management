...
describe("ListVehicleInsurances", () => {
  it("uses active records, twenty rows and page one by default", async () => {
    const result = { insurances: [], totalCount: 0 }; const reader = { search: vi.fn(async () => result), findById: vi.fn(async () => null) };
  ...
  it("normalizes search and bounds paging while preserving all/inactive selection", async () => {
    const reader = { search: vi.fn(async () => ({ insurances: [], totalCount: 0 })), findById: vi.fn(async () => null) }; const list = new ListVehicleInsurances(reader);
  ...
  it("propagates read failures", async () => {
    ...
    await expect(new ListVehicleInsurances({ search: vi.fn().mockRejectedValue(error), findById: vi.fn(async () => null) }).execute()).rejects.toBe(error);
  });
