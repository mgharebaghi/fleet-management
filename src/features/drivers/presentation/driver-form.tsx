...
import { FormField, FieldLabel, FieldHint, FieldErrors, FormActions, FormSection, formControlClassName } from "../../../components/ui/form-field/form-field";
...
type CurrentVehicleAssignment = { vehicleId: number; assignmentId: number };
type Props = { kind: "driver"; people: PersonReference[] } | { kind: "license"; driverId: number } | { kind: "editLicense"; driverId: number; licenseId: number; initialValues: Record<string, string> } | { kind: "assignment"; driverId: number; vehicles: VehicleReference[]; currentAssignments: readonly CurrentVehicleAssignment[] } | { kind: "editAssignment"; driverId: number; assignmentId: number; vehicles: VehicleReference[]; currentAssignments: readonly CurrentVehicleAssignment[]; initialValues: Record<string, string> } | { kind: "close"; driverId: number; assignmentId: number };
...
const titles = { driver: "تعریف راننده", license: "ثبت گواهینامه", editLicense: "ذخیره تغییرات گواهینامه", assignment: "تخصیص خودرو به راننده", editAssignment: "ذخیره تغییرات تخصیص", close: "ثبت پایان تخصیص" };
...
export function DriverForm(props: Props) {
  ...
  const vehicleError = state.error === "VEHICLE_CURRENTLY_ASSIGNED" || state.error === "VEHICLE_INACTIVE" || state.error === "VEHICLE_NOT_FOUND" ? driverMessages[state.error] : null;
  const vehicleHintId = `${prefix}-vehicle-hint`;
  const vehicleErrorId = `${prefix}-vehicle-error`;
  const options = props.kind === "driver" ? props.people.map(p => ({ value: String(p.personId), label: `${p.firstName} ${p.lastName} ${p.personnelNo ?? ""} ${p.nationalCode ?? ""}`, searchText: `${p.firstName} ${p.lastName} ${p.personnelNo ?? ""} ${p.nationalCode ?? ""}`, content: <span>{p.firstName} {p.lastName} — <TechnicalValue>{p.personnelNo ?? p.nationalCode ?? String(p.personId)}</TechnicalValue></span> })) : assignmentKind ? buildAssignmentVehicleOptions(props.vehicles, props.currentAssignments, props.kind === "editAssignment" ? props.assignmentId : undefined) : [];
  const selectable = options.some(option => !("disabled" in option && option.disabled));
  ...
  return <form action={formAction} noValidate aria-busy={pending} aria-label={titles[props.kind]} aria-describedby={state.error ? `${prefix}-error` : undefined} className={styles.form}>
    ...
    <FormSection title={titles[props.kind]}>
    <FormGrid>
      {(props.kind === "driver" || assignmentKind) && <FormField>
        <SearchableSelect name={props.kind === "driver" ? "personId" : "vehicleId"} label={props.kind === "driver" ? "شخص" : "خودرو"} options={options} defaultValue={value(props.kind === "driver" ? "personId" : "vehicleId")} disabled={pending} required
          ...
          invalid={assignmentKind && vehicleError !== null}
          describedBy={assignmentKind ? `${vehicleHintId}${vehicleError ? ` ${vehicleErrorId}` : ""}` : undefined} />
        {assignmentKind && <FieldHint id={vehicleHintId}>خودرویی که همین الان تخصیص جاری دارد با برچسب «در دسترس نیست» نشان داده می‌شود و قابل انتخاب نیست. جستجو با نام و پلاک همچنان کار می‌کند.</FieldHint>}
        {assignmentKind && vehicleError && <FieldErrors id={vehicleErrorId} messages={[vehicleError]} />}
      </FormField>}
    ...
    <FormActions separated={props.kind !== "driver"}><ActionButton type="submit" disabled={pending || ((props.kind === "driver" || assignmentKind) && !selectable)} pending={pending}>{pending ? "در حال ذخیره…" : titles[props.kind]}</ActionButton></FormActions>
  </form>;
}
